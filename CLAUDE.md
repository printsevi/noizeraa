...
The **code monorepo skeleton exists but is unimplemented** — `apps/api` is a bare NestJS app with only a `/healthz` endpoint, `apps/web` is a bare Next.js app with a placeholder page, and `packages/{contracts,domain,infra}` are empty export stubs (infra has a `shared/` stub for the module layout below). Dependencies are installed and `pnpm build/lint/typecheck/test` are green, but no feature-level business logic exists yet. `infra/` (OpenTofu + cloud-init + Compose) is scaffolded as source only — **no `tofu apply` has ever been run**, no cloud resources exist. Known gaps between the tech proposal and the scaffold are tracked as `docs/tickets/001`–`008`.
...
- `docs/brief/tech-proposal/noizera-technical-overview.md` — the technical implementation plan (v1.1; errata listed under its header), written as a companion to the pitch — **read this in full before writing any feature code.** It is a detailed, decision-level spec (stack choices with rejected alternatives, data model, sequencing) — not a vague proposal. Treat its "Decided"/"Given" items as settled unless the user says otherwise, and its "Suggested" items as defaults open to revisiting.
- `docs/adrs/`, `docs/specs/`, `docs/tickets/`, `docs/bugs/`, `docs/tech-debt/`, `docs/glossary.md` — see "Doc categories" below.

## Commands

pnpm workspaces + Turborepo, pinned via `packageManager` in the root `package.json`. `pnpm install` also installs the git hooks (lefthook: lint-staged on pre-commit, commitlint on commit-msg — conventional commits, types in `commitlint.config.mjs`). On this Windows machine `corepack enable` needs admin; use `corepack pnpm …` or the shim in `~/.corepack-bin` instead.

```bash
pnpm dev                                                       # apps/api + apps/web, via turbo
pnpm build                                                     # all packages, via turbo (packages/* build to dist/, which apps consume)
pnpm lint                                                      # eslint per package + `pnpm lint:deps`
pnpm lint:deps                                                 # dependency-cruiser module/package boundary rules (.dependency-cruiser.cjs)
...
pnpm format                                                    # prettier --write (docs/brief is excluded on purpose)
```
...
Workspace packages are imported from their built `dist/` (`exports` maps in each `package.json`); a fresh clone needs `pnpm build --filter='./packages/*'` before a single-package `test`/`lint` can resolve `@noizera/*`. `@noizera/infra` additionally exposes `@noizera/infra/shared` and `@noizera/infra/<module>` subpaths — see "Architecture".
...
packages/
  ...
  infra/        Drizzle schema + repositories, S3 client, AMQP client
                  src/shared/    db client, outbox, processed_messages, S3, AMQP  → @noizera/infra/shared
                  src/<module>/  that module's schema + repositories               → @noizera/infra/<module>
```
...
These boundaries are **enforced by `pnpm lint:deps`** (`.dependency-cruiser.cjs`), not just described: `domain` imports nothing from the workspace or any framework; `contracts` imports only `zod`; `web` imports only `@noizera/contracts`; `apps/api/src/<m>` may import `@noizera/infra/<m>` and `@noizera/infra/shared` but no other module's infra subpath, and may reach another api module only through that module's `index.ts`. A violation is a design error to resolve in `grill-me`, not a rule to suppress.
...
- **`.github/workflows/ci-cd.yml`** — format/lint/typecheck/test/e2e on every push and PR; on push to `main` only, after all of those pass: build + push both images to GHCR, then deploy to vm-app over Tailscale (ADR 001): rsync `infra/compose`, then tech proposal §12's exact SSH sequence. `workflow_dispatch` with a `sha` input redeploys an already-built image (rollback). The deploy job will fail until vm-app is provisioned, joined to the tailnet, and holds `/opt/noizera/age.key` + `/opt/noizera/infra/compose/.env.enc`, and until `dist/migrate.js` exists (`docs/tickets/003`) — scaffolded to match the target design, not tested end to end.
- **`.github/workflows/infra-plan.yml`** — `tofu fmt/validate/plan` only, on PRs touching `infra/tofu/**` or `infra/cloud-init/**`; posts the plan as a PR comment.
...
All workflows declare least-privilege `permissions:` and pin actions to commit SHAs (Dependabot keeps them current — don't hand-edit a SHA without updating the version comment). Both infra workflows decrypt/re-encrypt `infra/tofu/terraform.tfstate.enc` with SOPS+age around `tofu init` (tech proposal §11.3's local-state decision means CI has to carry state across runs itself, not rely on a remote backend) — the apply workflow commits the re-encrypted state back to the repo after applying.

Secrets these workflows expect (none are configured yet): `HCLOUD_TOKEN`, `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ZONE_ID`, `HETZNER_S3_ACCESS_KEY`, `HETZNER_S3_SECRET_KEY`, `DEPLOY_SSH_PUBLIC_KEY`, `ADMIN_IP`, `SOPS_AGE_KEY` (private), `SOPS_AGE_PUBLIC_KEY`, `TS_OAUTH_CLIENT_ID`, `TS_OAUTH_SECRET`, `VM_APP_TAILSCALE_HOST`, `VM_APP_SSH_USER`, `VM_APP_SSH_KEY`. `GITHUB_TOKEN` (for GHCR push) is automatic.
...
A third, installed skill — `playwright-best-practices` (from currents-dev, vendored under `.claude/skills/`) — is general Playwright guidance; the `tdd` skill's "Playwright for the listener flow only" rule wins where they differ.

Default sequence for a new feature: `grill-me` first, then a ticket under `docs/tickets/` if it's non-trivial, then `tdd`.

## Claude Code hooks

`.claude/settings.json` (committed) wires two hooks in `.claude/hooks/`: `guard-bash.mjs` (PreToolUse on Bash) refuses `tofu apply|destroy`, `drizzle-kit push`, `git push --force`, `docker compose down -v` and recursive deletes outside the scratchpad — these are user-run-only actions, so if one is genuinely wanted, ask the user to run it; `format.mjs` (PostToolUse on Edit/Write) runs prettier on the touched file. A read-only permission allowlist there keeps `git status/diff/log` and `pnpm lint/typecheck/test` prompt-free.
